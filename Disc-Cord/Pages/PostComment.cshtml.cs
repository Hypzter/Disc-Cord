...
namespace Disc_Cord.Pages
{
    public class PostCommentModel : PageModel
    {
            ...
            int deletepostid, int deletecommentid, bool deletebool, int unflagpostid, int unflagcommentid, int? pageIndex)
        {
            ...
            if (commentid != 0)
            {
                await AddCommentLikeAsync(commentid, userid);
            }
            ...
            {
                await UnFlagAsync(unflagpostid, unflagcommentid);
            }
        ...
        public async Task<IActionResult> OnPostAsync(int editpostid, int editcommentid, bool editbool, bool newcommentbool, int reportpostid, int reportcommentid)
        {
            ...
            {
                await EditCommentAsync(editcommentid, editbool);
            }
            ...
            {
                await AddNewcommentAsync(newcommentbool);
            }
        ...
        private async Task<IActionResult> AddNewcommentAsync(bool newcommentbool)
        {
        ...
        private async Task<IActionResult> EditCommentAsync(int editcommentid, bool editbool)
        {
        ...
        private async Task<IActionResult> ReportAsync(int reportpostid, int reportcommentid)
        {
            if (reportpostid != 0)
            {
        ...
        private async Task UnFlagAsync(int unflagpostid, int unflagcommentid)
        {
            if (unflagpostid != 0)
            {
        ...
        private async Task<IActionResult> DeleteCommentAsync(int deletecommentid, bool deletebool)
        {
            ...
            deletebool = false;
            var url = ("/PostComment?id=" + _id);
            return Redirect(url);
        }

        private async Task<IActionResult> DeletePostAsync(int deletepostid, bool deletebool)
        {
        ...
        private async Task<IActionResult> AddCommentLikeAsync(int commentid, string userid)
        {
